    ...
    <div style={styles.container}>
      {acceptedTeams.length > 0 ? (
        <div style={styles.grid}>
          {acceptedTeams.map((t) => (
            <div key={t.id} style={styles.card} onClick={() => router.push(`/teams/member/${t.id}`)}>
              <h3 style={styles.title}>{t.name}</h3>
              <p style={styles.muted}><strong>Leader:</strong> {t.leader}</p>
              <p style={styles.muted}><strong>Society:</strong> {t.society}</p>
            </div>
          ))}
        </div>
      ) : (
        <p style={styles.empty}>No teams joined yet. Accept an invitation to get started!</p>
      )}
    </div>
  );
}
...
const styles: any = {
  ...
  card: { background: "#2a2a2a", color: "#fff", padding: 18, borderRadius: 12, cursor: "pointer", boxShadow: "0 6px 18px rgba(0,0,0,0.4)" },
